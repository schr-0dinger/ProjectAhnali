...
- Track C Wave 3 adds tokened async route dispatch (`http_get_route_async`) with deterministic cancellation/progress/error/status/body surfaces (`http_async_cancel`, `http_async_progress`, `http_async_error`, `http_async_status`, `http_async_body`), progress callback wiring, and timeout/retry controls via helper async starter/state methods and runnable support classes.
